// Workaround for a Next.js build-time cost on `output: 'export'` builds.
//
...
// is the single biggest serial phase of the build and, because @vercel/nft opens the whole dependency
// graph at once, it holds thousands of file descriptors concurrently. On hosts with a low open-file
// limit that surfaces as an `EMFILE: too many open files` crash during the build. Next 14 let you skip
// the trace with `outputFileTracing: false`; Next 15+ removed that option and ships no replacement.
//
...
try {
  ...
  console.warn(
    "[skip-export-traces] next/dist/build/index.js not resolvable — skipping (build still works)"
  );
  process.exit(0);
}
